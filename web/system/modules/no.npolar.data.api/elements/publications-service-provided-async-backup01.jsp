        ...
        <form class="search-panel" action="<%= cms.link(requestFileUri) %>" method="get" id="pub-search-form">
            ...
            %>
                <div class="layout-group single layout-group--single filter-widget" style="text-align:right;">
                    <a href="<%= cms.link(requestFileUri) %>" style="display:inline-block; width:auto;" class="cta cta--alt">
...
<script type="text/javascript">
    // Define the provider (of the updated search view)
    var provider = '<%= cms.link("/system/modules/no.npolar.data.api/elements/publications-loader-async.jsp") %>';
    
    // Add the modifier class .filter--async to all .filter links
    $('#pub-search a.filter').addClass('filter--async');
    
    $('#pub-search').on('click', '.filter--async', function(event) {
        event.preventDefault();
        var filterTarget = $(this).attr('href');
        var queryString = getQueryString(filterTarget);// = filterTarget.substring(filterTarget.indexOf("?")+1);
        queryString += '&base=<%= requestFileUri %>';
        // TODO: Switch to using ID (which we need anyway, for "aria-controls")
    ...
    var updateAsync = function(provider, queryString) {
        ...
        console.log("looooading URL: " + provider + "?" + queryString);
        
        $('body').append('<div class="overlay--loading" id="pub-search-overlay" style="position:fixed; top:0; bottom:0; left:0; right:0; background-color:rgba(0,0,0,0.5); z-index: 9999;"></div>');
