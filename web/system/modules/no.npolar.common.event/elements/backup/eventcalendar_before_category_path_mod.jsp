...
if (displayType > 1) {
    ...
    if (CmsAgent.elementExists(hostCategoryPath)) {
        ...
        while (hostItr.hasNext()) {
            ...
            String hostCatPath = hostCat.getRootPath();
...
if (displayType > 2 && displayType < 5) {
    ...
    if (allCatItr.hasNext()) {
            ...
            if (topLevelCategories.indexOf(cat) > -1 && !hideCategoryFilter) {
                ...
                if (iSubCat.hasNext()) {
                    ...
                    while (iSubCat.hasNext()) {
                        ...
                        if (parameterString.contains(subCat.getRootPath())) {
                            String removeLink = "<a class=\"remove\" href=\"" + cms.link(OVERVIEW_FILE) + "?" + parameterString;
                            removeLink = removeLink.replace(PARAM_NAME_CATEGORY + "=".concat(subCat.getRootPath()), "");
                        ...
                        // Category is not present in the current request parameters: create a link to filter by this category
                            ...
                            catMatches.add(subCat.getRootPath());
                            int numCatMatches = 0;
                            try {
                            ...
                            if (numCatMatches > 0) {      
                                ...
                                String filter = sortComment +
                                            ...
                                            parameterString + "&amp;" + PARAM_NAME_CATEGORY + "=" + subCat.getRootPath() + "\" rel=\"nofollow\">" + 
        ...
        out.println("</div><!-- .event-links.nav -->");
    } else {
        out.println("<p>" + LABEL_NO_CATEGORIES + ".</p>");
    }
...
if (displayType > 0) {
    ...
    if (!datedEvents.isEmpty()) {
        ...
        if (range == EventCalendar.RANGE_CURRENT_DATE) {
            ...
            datedRangeListHeading = headingDate.format(calendar.getTime());
        }
            ...
            datedRangeListHeading = Integer.toString(calendar.get(Calendar.YEAR));
        }
        else if (range == EventCalendar.RANGE_CURRENT_WEEK) {
            datedRangeListHeading = cms.label("label.for.np_event.week") + " " + calendar.get(Calendar.WEEK_OF_YEAR) + " " + calendar.get(Calendar.YEAR); 
        }
        ...
        if (displayType < 1)
            out.println("<h3>" + datedRangeListHeading + "</h3>");

        String inRangeHeading = range == EventCalendar.RANGE_CURRENT_DATE ? (LABEL_STARTS + " ") : (range == EventCalendar.RANGE_CATCH_ALL ? "" : (LABEL_STARTS_IN + " ")) + datedRangeSubListHeading;
        ...
        if (displayType > 1) { // Option 1: as a table
            ...
            if (!eventsStartingInRange.isEmpty()) {
                out.println("<h3>" + inRangeHeading + "</h3>");
            ...
            if (!eventsOverlappingRange.isEmpty()) {
                out.println("<h3>" + LABEL_IN_PROGRESS + "</h3>");
            ...
            if (!eventsExpired.isEmpty() && range != EventCalendar.RANGE_CATCH_ALL) {
                out.println("<h3>" + expiredHeading + "</h3>");
