...
if (cms.template("list")) {
    ...
    if (!menuItems.isEmpty()) {
        out.println("<nav id=\"quicklinks\" class=\"fullwidth-centered clearfix\">");
        ...
        while (i.hasNext()) {
            mi = (MenuItem)i.next();
            html = "<li>";
            html += "<a href=\"" + cms.link(mi.getUrl()) + "\" class=\"quicklink\">" + mi.getNavigationText().replaceAll(" & ", " &amp; ").trim() + "</a>";
...
if (cms.template("dropdown")) {
    out.println("select) -->");

    if (!menuItems.isEmpty()) {
        out.println("<select name=\"navtarget\" onchange=\"submit()\">");
        out.println("<option value=\"\">" + (loc.equalsIgnoreCase("no") ? "Snarveier" : "Shortcuts") + "</option>");
        MenuItem mi = null;
        String html;
        Iterator i = menuItems.iterator();
        while (i.hasNext()) {
            ...
            html = "<option"
                        ...
                        + " value=\"" + mi.getUrl() + "\""
                    + ">" 
                        + mi.getNavigationText().replaceAll(" & ", " &amp; ").trim() 
                    + "</option>";
            out.print(html);
        }
        out.println("</select>");
