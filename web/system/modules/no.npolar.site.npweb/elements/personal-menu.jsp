...
public String getResourceList(List resourceList, CmsObject cmso, CmsAgent cms) throws CmsException {
    ...
    if (resourceList.size() > 0) {
        ...
        while (iFolderResources.hasNext()) {
            ...
            html += "<li><a href=\"" + cms.link(pagePath) + "\"" + (cms.getRequestContext().getUri().equals(pagePath) ? " class=\"current\"" : "") + ">" + pageTitle + "</a></li>";
        }
        html += "</ul>";
        html += "</div>";
    }
...
try {
    ...
    if (resourcesInFolder.size() > 0) {
        // Found at least 1 page, so create a list
        String personName = cms.property("Title", personFolderPath, "");

        out.println("<aside>"); // Wrapper
        out.println("<h4>" 
                        + "<a href=\"" + cms.link(personFolderPath) + "\">" + personName + "</a>" // Link to the person's main page
                        + "'" + (personName.endsWith("s") || personName.endsWith("z") ? "" : "s") 
                        + (loc.equalsIgnoreCase("no") ? " sider" : " pages") 
                    + "</h4>"); // Navigation heading

        // Print personal pages in the main folder
        out.println(getResourceList(resourcesInFolder, cmso, cms));

        // Print personal pages in subfolders
    ...
    // person / personalpage not installed
}
%>