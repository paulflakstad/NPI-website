<%-- 
    Document   : polar-bokkafe-list
    Created on : May 16, 2012, 11:32:23 AM
    Author     : flakstad
...
public String getEventsOverview(CmsAgent cms, String heading, List events, SimpleDateFormat df, Date nextDateIfNone, boolean fadePastEvents) throws CmsException {
    ...
    final String LABEL_NONE = "no".equalsIgnoreCase(cms.getRequestContext().getLocale().toString()) ? "Ingen" : "None";
    ...
    if (!events.isEmpty()) {
        ...
        if (nextDateIfNone != null) {
            html += "<em>, "
                    + ("no".equalsIgnoreCase(cms.getRequestContext().getLocale().toString()) ? "neste er" : "next is") 
                    + df.format(nextDateIfNone) 
                    + "</em>";
        }
    }
...
// Loop over the categories, and remove all that are not "Polar book cafe"
...
while (allCatItr.hasNext()) {
    ...
    //if (category.getTitle().equalsIgnoreCase(CATEGORY_TITLE_BOOK_CAFE)) {
    if (category.getPath().equalsIgnoreCase(CATEGORY_PATH_BOOK_CAFE)) {
        // add category path
...
// Get today's events that are labelled "Polar book cafe"
...
// Get the next two events that are labelled "Polar book cafe"
...
out.println(cms.getContent(requestFolderUri+"polar-bokkafe-bottom.html", "body", locale));
