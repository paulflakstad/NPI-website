...
public static String getCompendiaPersonalSerp(String usr, String pwd, String query) throws Exception {
    ...
    String pathSerp = "/kunder/npolar/sokemoto.nsf/sokemotorxml?OpenAgent&search=" + query;
    int    port = 80;
    ...
    try {
        ...
        if (!items.isEmpty())
            s += "<a href=\"" + protocol + "://" + domain + pathSerp + "\">";
        ...
        if (!items.isEmpty()) {    
            /*
            s += "<p><a href=\"" + protocol + "://" + domain + pathSerp + "\">";
            s += "Se treff" + (numHits > 1 ? "ene" : "et") + " i personalhåndbokas søk";
            s += "</a></p>";
            if (numHits > topHitsLimit) {
                s += "<p style=\"font-weight:bold;\">Topptreff:</p>";
            }
            //*/
            s += "<ul id=\"comp-pers-serp\" style=\"list-style:none; padding-left:0;\">";
            Iterator i = items.iterator();
            int itemsVisited = 0;
            while (i.hasNext() && itemsVisited++ < topHitsLimit) {
                ...
                String url = "http://www.compendiapersonal.no/kunder/npolar/ph.nsf/unique/" + idNode.getText();
                s += "<li>";
            ...
            if (topHitsLimit < numHits) {
                ...
                s += "<a href=\"" + protocol + "://" + domain + pathSerp + "\">&hellip;&nbsp;og " + diff + (diff > 1 ? " flere treff" : " treff til") + "</a>";
            }
...
<%
	...
	*/
    ...
    <%
        ...
        <h2><%= search.getSearchResultCount() %><%= (loc.equalsIgnoreCase("no") ? 
        " treff for " : " hits for ")%><i><%= search.getQuery()%></i><%= (loc.equalsIgnoreCase("no") ? 
        " på Isblink" : " on Isblink")%></h2>
        <%
...
if (q != null && !q.isEmpty()) {
    ...
    out.println("<div class=\"\" style=\"float:right; width:49%;\">");
