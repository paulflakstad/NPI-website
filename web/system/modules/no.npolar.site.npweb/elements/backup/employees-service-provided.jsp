<%-- 
    Document   : employees-service-provided
...
final static String PARAM_NAME_SEARCHPHRASE_SERVICE = "q-first_name,last_name";
...
public String getFacetParameterString(String url) {
    ...
    Map defaultParamMap = getPreDefParamMap();
    // Other parameters to hide?
    defaultParamMap.put("facets", ""); // Don't want the "facets" parameter in filter URLs
    defaultParamMap.put("start", ""); // Don't want the "start" parameter in filter URLs
    ...
    for (int i = 0; i < parameters.length; i++) {
        ...
        if (!defaultParamMap.containsKey(k)) {
            if (PARAM_NAME_SEARCHPHRASE_SERVICE.equals(k)) {
    ...
    if (!pm.isEmpty()) {
        ...
        while (pNames.hasNext()) {
            ...
            if ("start".equals(pName) && !includeStart
                ...
                continue; // Ignore this if this is the "start" parameter and we've decided to override that, OR if this is a pre-defined parameter
            String pValue = "";
...
final String LABEL_SEARCH_PH        = loc.equalsIgnoreCase("no") ? "Søk på navn" : "Search by name";//cms.label("label.np.search");
    ...
    if (totalResults > 0) {
        ...
        <table class="odd-even-table">
            ...
            for (int pCount = 0; pCount < entries.length(); pCount++) {
                ...
                try {
                    String thisNameLetter = getSortChar(lName);
