...
params.put("filter-published-year", new String[] { year }); // Filter on the given year
...
// Parameters used only by machines
...
params.put("format", new String[]{ "json" }); // Tell the service to respond in JSON
params.put("limit", new String[]{ Integer.toString(LIMIT) }); // Set an entry limit
...
try {
    ...
    for (int pubCount = 0; pubCount < pubs.length(); pubCount++) {
        ...
        if (COMMENTS) out.println("<!-- Added publication: " + id + " (" + CmsStringUtil.trimToSize(title, 40) + ") -->");
    }
} catch (Exception e) {
...
if (!publications.isEmpty()) {
    //out.println("<h2>" + cms.labelUnicode("label.np.publist.heading") + "</h2>");
    ...
    while (iTypes.hasNext()) {
        ...
        if (iPubs.hasNext()) {
            ...
            out.println("<h2 class=\"toggletrigger\""
                            //+ " href=\"javascript:void(0);\""
                        + ">"                    
                                + cms.labelUnicode("label.np.pubtype." + listType) + " (" + publications.getListByType(listType).size() + ")"
                        + "</h2>");
