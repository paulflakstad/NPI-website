...
%><%   
    ...
    */
    ...
    String searchFieldTitle = loc.equalsIgnoreCase("no") ? "Søk på nettstedet" : "Site search";
    String searchButtonTitle = loc.equalsIgnoreCase("no") ? "Søk" : "Search";
%>
    ...
    <jsp:setProperty name="search" property="*"/>
        <% search.init(cms.getCmsObject()); %>
                        ...
                        <form method="get" action="<%= cms.link(serp) %>">
                            <input title="<%= searchFieldTitle %>" type="search" class="query" name="query" id="query" tabindex="3" placeholder="<%= defaultSearchText %>" />
                            <!--<input type="hidden" name="index" value="<%= indexName %>" />-->
                            <!--<input type="submit" class="submit" value="" tabindex="4" />-->
                            <button title="<%= searchButtonTitle %>" type="submit" class="submit" value="" tabindex="4"><i class="icon-search"></i></button>
                        </form>
                        <script type="text/javascript">
                            //$("#query").autofill({value: '<%= defaultSearchText %>'});// requires jQuery with the autofill plugin
                        </script>