...
final String FOLDER = "/no/forskning/ice/prosjekt/fimbulisen/nare0910/expedition-diary/entries/";
...
final String XML_ELEMENT_NAME       = "Paragraph/Image/URI";

final String OLD_VALUE              = "/sites/fimbul/images/";
...
while (iFilesInFolder.hasNext()) {
    ...
    try {
        ...
        List xmlContentElementValues = xmlContent.getValues("TeaserImage", locale);
        ...
        if (iValues.hasNext()) { // use if instead of while, because there should be only one value
            ...
            if (valueString.startsWith(OLD_VALUE)) {
                String replacementValueString = "/images/ice/" + valueString.replace(OLD_VALUE, "");
                out.println(" -- Replacement: " + replacementValueString + "<br />");
        ...
        int i = 0;
        while (i < 100) { // Paragraph loop
            i++;
            int j = 0;
            while (j < 100) { // Paragraph/Image loop
                xmlContentElementValues = xmlContent.getValues("Paragraph[" + (i) + "]/Image[" + (++j) + "]/URI", locale);
                iValues = xmlContentElementValues.iterator();
                if (iValues.hasNext()) { // use if instead of while, because there should be only one value
                    I_CmsXmlContentValue value = (I_CmsXmlContentValue)iValues.next();
                    String valueString = value.getStringValue(cmso);
                    out.println(" - Value: " + valueString + "<br />");
